...
// 🔹 extended with type + avgPrice so filters can use them
const nearby = [
  {
    id: "r1",
    name: "Joe's Diner",
...
export default function HomeScreen() {
  ...
  // filter panel + restaurant detail state
  const [showFilter, setShowFilter] = useState(false);
  const [minRating, setMinRating] = useState<number | null>(null);
  const [maxDistance, setMaxDistance] = useState<number | null>(null);

  // NEW: extra filters
  const [typeFilter, setTypeFilter] = useState<string | null>(null);       // food, drink, snacks
  const [locationFilter, setLocationFilter] = useState<string | null>(null); // Ikeja, VI, etc.
  const [maxPrice, setMaxPrice] = useState<number | null>(null);          // simple price cap

  const [selectedRestaurant, setSelectedRestaurant] = useState<Restaurant | null>(null);
  const [activeRestaurantTab, setActiveRestaurantTab] = useState<"order" | "review" | "info">("order");

  // active foods for the chips/categories row
  const activeFoods = useMemo(() => {
    if (selectedCategory === "all") return foods;
    return foods.filter((f) => f.type === selectedCategory);
  }, [selectedCategory]);

  // filter nearby restaurants based on search + filters
  ...
  const toggleLocation = (value: string) => {
    setLocationFilter((prev) => (prev === value ? null : value));
  };

  const togglePrice = (value: number) => {
    setMaxPrice((prev) => (prev === value ? null : value));
  };
  ...
  // ---------- RESTAURANT DETAIL "PAGE" (within Home) ----------
              ...
              {activeRestaurantTab === "review" && (
                <View>
                  ...
                  <Text style={{ marginTop: 8, color: "#6B7280" }}>
                    ★★★★☆ 4.5 average based on 120 reviews.
                  </Text>
                  <Text style={{ marginTop: 8 }}>
                    “Great food and quick delivery!” — User A
                  </Text>
                  <Text style={{ marginTop: 4 }}>
                    “Loved the ambience and the menu variety.” — User B
                  </Text>
                </View>
              )}
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: "#fff" }}>
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: 80 }}>
        <View style={styles.container}>
            ...
            <View style={styles.filterPanel}>
              ...
              <View style={styles.filterRow}>
                ...
                <View style={styles.filterChipsRow}>
                  <Pressable
                    onPress={() => toggleRating(4.0)}
                    style={[
                      styles.filterChip,
                      minRating === 4.0 && styles.filterChipActive,
                    ]}
                  >
                    <Text
                      style={[
                        styles.filterChipText,
                        minRating === 4.0 && { color: "#fff" },
                      ]}
                    >
                      4.0+
                    </Text>
                  ...
                  <Pressable
                    onPress={() => toggleRating(4.5)}
                    style={[
                      styles.filterChip,
                      minRating === 4.5 && styles.filterChipActive,
                    ]}
                  >
                    <Text
                      style={[
                        styles.filterChipText,
                        minRating === 4.5 && { color: "#fff" },
                      ]}
                    >
                      4.5+
                    </Text>
              ...
              <View style={styles.filterRow}>
                ...
                <View style={styles.filterChipsRow}>
                  <Pressable
                    onPress={() => toggleDistance(0.5)}
                    style={[
                      styles.filterChip,
                      maxDistance === 0.5 && styles.filterChipActive,
                    ]}
                  >
                    <Text
                      style={[
                        styles.filterChipText,
                        maxDistance === 0.5 && { color: "#fff" },
                      ]}
                    >
                      {"< 0.5km"}
                    </Text>
                  ...
                  <Pressable
                    onPress={() => toggleDistance(1.0)}
                    style={[
                      styles.filterChip,
                      maxDistance === 1.0 && styles.filterChipActive,
                    ]}
                  >
                    <Text
                      style={[
                        styles.filterChipText,
                        maxDistance === 1.0 && { color: "#fff" },
                      ]}
                    >
                      {"< 1km"}
                    </Text>
              ...
              <View style={styles.filterRow}>
                ...
                <View style={styles.filterChipsRow}>
                  <Pressable
                    onPress={() => togglePrice(5)}
                    style={[
                      styles.filterChip,
                      maxPrice === 5 && styles.filterChipActive,
                    ]}
                  >
                    <Text
                      style={[
                        styles.filterChipText,
                        maxPrice === 5 && { color: "#fff" },
                      ]}
                    >
                      {"< $5"}
                    </Text>
                  ...
                  <Pressable
                    onPress={() => togglePrice(8)}
                    style={[
                      styles.filterChip,
                      maxPrice === 8 && styles.filterChipActive,
                    ]}
                  >
                    <Text
                      style={[
                        styles.filterChipText,
                        maxPrice === 8 && { color: "#fff" },
                      ]}
                    >
                      {"< $8"}
                    </Text>
                ...
                return (
                  <Pressable
                    ...
                    style={[
                      ...
                      active ? styles.consumableActive : styles.consumableInactive,
                    ]}
                  >
                    <Image source={{ uri: item.img }} style={styles.consumableImg} />
                    <Text style={[styles.consumableText, active && { color: "#fff" }]}>
                      {item.name}
                    </Text>
                  </Pressable>
                );
          ...
          {/* Menu for active category (first horizontal list + second group) */}
          ...
          {/* Nearby restaurants (vertical, clickable) */}
