// app/(auth)/(order)/order/[id].tsx
import React, { useEffect, useState } from "react";
import { ActivityIndicator, Alert, FlatList, Pressable, ScrollView, StyleSheet, Text, View } from "react-native";
import { Stack, useLocalSearchParams } from "expo-router";
import { SafeAreaView } from "react-native-safe-area-context";
import * as api from "../../../../src/shared/constants/api"; // adjust path if necessary
  ...
  useEffect(() => {
    ...
    const fetch = async () => {
      ...
      try {
        ...
        console.warn("fetch order err", err);
        if (mounted) Alert.alert("Error", err?.message || "Failed to load order");
      } finally {
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: "#fff" }}>
      ...
      <ScrollView contentContainerStyle={{ padding: 16 }}>
        <View style={{ marginBottom: 12 }}>
          ...
          <Text style={{ color: "#6B7280", marginTop: 6 }}>{new Date(order.createdAt).toLocaleString()}</Text>
        </View>
            ...
            renderItem={({ item }: any) => (
              <View style={{ flexDirection: "row", justifyContent: "space-between", paddingVertical: 8 }}>
                <View style={{ flex: 1 }}>
                  ...
                  <Text style={{ color: "#6B7280" }}>{item.quantity} × ₦{item.price.toFixed(2)}</Text>
                </View>
                <Text style={{ fontWeight: "700" }}>₦{item.subtotal.toFixed(2)}</Text>
              </View>
            )}
            ListFooterComponent={() => (
              <View style={{ borderTopWidth: 1, borderTopColor: "#F3F4F6", paddingTop: 8, marginTop: 8 }}>
                <View style={{ flexDirection: "row", justifyContent: "space-between" }}>
                  ...
                  <Text style={{ fontWeight: "700" }}>₦{order.totalAmount.toFixed(2)}</Text>
                </View>
              </View>
            )}
          />
        ...
        <View style={[styles.card, { marginTop: 12 }]}>
          ...
          <Text>{address.city}{address.state ? `, ${address.state}` : ""}</Text>
          <Text>{address.phone ?? ""}</Text>
        </View>

        <View style={[styles.card, { marginTop: 12 }]}>
          <Text style={{ fontWeight: "700", marginBottom: 8 }}>Status</Text>
          <Text style={{ marginBottom: 8 }}>{order.status}</Text>
          <Text style={{ color: "#6B7280" }}>Payment: {order.paymentStatus}</Text>
        </View>
